...
;Do testow CMD odpalacz_wagi.exe "10.49.30.1/test/insert.php;Waga pieca od sciany;4;2;0"
...
If $CmdLine[0] == 0 Then
	MsgBox($MB_SYSTEMMODAL, "Blad uruchomienia", "Uruchom przez gui.exe", 10)
...
Global $adres_skryptu = $tablica_stringow[0]
Global $nazwa_wagi = $tablica_stringow[1]
Global $port_com = $tablica_stringow[2]
Global $id_wagi = $tablica_stringow[3]
Global $rodzaj_wagi = $tablica_stringow[4]
...
If (StringInStr($w1, "ES") == 0) Then
	MsgBox($MB_SYSTEMMODAL, "Blad polaczenia z waga", "Sprawdz porty COM", 10) ;Niema polaczenia miedzy waga a tym urzadzeniem. Sprawdz do ktorego portu jest podlaczona waga
...
If ($rodzaj_wagi == 0) Then
	...
	$Form1_1 = GUICreate($nazwa_wagi, 790, 259, 201, 339, -1, BitOR($WS_EX_TOPMOST, $WS_EX_WINDOWEDGE))
...
ElseIf ($rodzaj_wagi == 1) Then
	...
	$Cofnij_button = GUICtrlCreateButton("Przerwij wazenie", 496, 192, 281, 57)
	GUICtrlSetFont(-1, 30, 400, 0, "MS Sans Serif")
...
While 1
	...
	GUICtrlSetData($work_status_label, $SIEC & " " & $WAGA)
...
Func HttpPost($sURL, $sData = "")
	...
	If ($oHTTP.Status <> $HTTP_STATUS_OK) Then
		wyjdz(3, 0, 0)
	Else
		GUICtrlSetData($work_status_label, "Wyslano: "&$dane)
	EndIf
...
Func cofnij($sServerAddress, $numer_czujnika)
	$Request = "nazwa=" & $numer_czujnika & "dane=" & $ostanie_dane_nadane
	HttpPost($sServerAddress, $Request)
EndFunc   ;==>cofnij
